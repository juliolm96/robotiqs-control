...
class FormSelect extends React.Component<PropTypes, MyState> {
  ...
  onChange(event: any) {
    ...
    if (this.props.name === 'optionsDias' && event.target.value !== '-- SELECCIONE UNA OPCION --') {
      ...
      HorarioServicioActions.setDia(event.target.value.charAt(0) + event.target.value.charAt(1));
    } else if (this.props.name === 'optionshh'  && event.target.value !== '-- SELECCIONE UNA OPCION --') {
      AsignacionHorarioActions.setHorarioId(event.target.value);
    }
