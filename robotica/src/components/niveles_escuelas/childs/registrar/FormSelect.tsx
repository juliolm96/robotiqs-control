...
import FormSelectOption from '../../../general/FormSelectOption0';
...
class FormSelect extends React.Component<PropTypes, MyState> {
  ...
  componentWillMount() {
    ...
    for (var i = 0; i < this.props.options.length; i++) {
      rows.push(<FormSelectOption key={i} option={this.props.options[i].nombre} value={this.props.options[i].id}/>);
    }
    this.setState({
      options: rows
    });
  ...
  onChange(event: any) {
    this.opt1 = true;
    if (this.props.name === 'Escuelas') {

      this.setState({
        idEscuela: 1
      });

      NivelEscuelaActions.recuperarNivEscuelas(event.target.value);
      NivelEscuelaActions.setIdEscuela(event.target.value);
    }
